...
class MiscHelper extends Helper {
  ...
  timeSince(time, compact = false, locale = 'en-gb') {
    ...
    if (Math.floor(timeSeconds) > 0) {
      text += `**${Math.floor(timeSeconds)}**${i18n.__(`dateUtil.${compact ? 'compact' : 'full'}.seconds`)}`;
    }

    return text ? text.trim() : i18n.__('global.noTimeAgo'); // 'no time ago' assuming it's less than a second ago
  }
}
